...
export default function Ressources() {
  ...
  const recupererSuggestion = async () => {
    if (estConnecte) {
      ...
      if (listeRessources.length > 0) {
        ...
        const plusRecente = [...listeRessources].sort((a, b) => new Date(b.creeLe).getTime() - new Date(a.creeLe).getTime())[0];
  ...
  const ressourcesAffichees = ressourcesFiltrees.slice((pageAffichee - 1) * RESSOURCES_PAR_PAGE, pageAffichee * RESSOURCES_PAR_PAGE);
  ...
  return (
    <div className="container-ressources" style={{ display: "flex", flexGrow: 1, flexDirection: "column" }}>
      ...
      <div className="ress-top" style={{ display: "flex", flexGrow: 1, flexDirection: "row" }}>
        ...
        {/* Colonne de gauche (Ressources d'aide & Filtres) */}
        <div className="ress-top-left b-rad25" style={{ display: "flex", flexGrow: 1, flexDirection: "column" }}>
          ...
          {!estConnecte && (
            <p>
              Connectez-vous pour ajouter des favoris! <i className="fa-solid fa-heart"></i>
            </p>
          )}
        </div>

        {/* Colonne de droite (Suggestion / Ressource récente) */}
        <div className="ress-top-right b-rad25" style={{ flexGrow: 1, display: "flex", flexDirection: "column" }}>
          <h2 style={{ justifyContent: "flex-end", paddingRight: "32px", flexGrow: 1, textAlign: "right" }}>{estConnecte ? "Suggéré pour vous" : "Ressource récente"}</h2>
              ...
              (() => {
                ...
                return (
                  <div key={ressSuggeree.id} onClick={() => window.open(ressSuggeree.url, "_blank")} className="card b-rad25" style={{ display: "flex", flexDirection: "column" }}>
                    ...
                    <h5 style={{ textAlign: "left", minHeight: "1.5rem", margin: "8px 0" }}>
                      Niveau <span className="duree">{ressSuggeree.niveau}</span>
                    </h5>
                    <p style={{ textAlign: "left", minHeight: "3rem", flexGrow: 1 }}>{ressSuggeree.contenu}</p>
                    <h4 style={{ textAlign: "left", minHeight: "1.5rem", margin: "8px 0" }}>
                      Durée: <span className="duree">{ressSuggeree.duree} minutes</span>
                    </h4>
                    <div className="card-fav-split" style={{ display: "flex", justifyContent: "center", alignItems: "center", flexGrow: 1 }}>
                      ...
                      <div className="card-fav-split-right" style={{ display: "flex", justifyContent: "center", alignItems: "center", flexGrow: 1 }}>
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            toggleFavori(ressSuggeree.id);
                          }}
                          disabled={!estConnecte}
                          hidden={!estConnecte}
                          title={estConnecte ? "" : "Connectez-vous pour ajouter au favoris."}
                        >
                          {estFavori ? <i className="fa-solid fa-heart fa-3x"></i> : <i className="fa-regular fa-heart fa-3x"></i>}
                        </button>
                      </div>
                    </div>
                  </div>
      ...
      <div className="ress-bot" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", flexGrow: 2, flexWrap: "wrap", gap: "8px" }}>
        ...
        {!chargement && ressourcesAffichees.length === 0 && <p>Aucune ressource ne correspond à votre recherche.</p>}
        {ressourcesAffichees.map((r: any) => {
          ...
          return (
            <div key={r.id} className="card b-rad25" onClick={() => window.open(r.url, "_blank")} style={{ display: "flex", flexDirection: "column" }}>
              ...
              <h5 style={{ textAlign: "left", minHeight: "1.5rem", margin: "8px 0" }}>
                Niveau <span className="duree">{r.niveau}</span>
              </h5>
              <p style={{ textAlign: "left", minHeight: "3rem", flexGrow: 1 }}>{r.contenu}</p>
              <h4 style={{ textAlign: "left", minHeight: "1.5rem", margin: "8px 0" }}>
                Durée: <span className="duree">{r.duree} minutes</span>
              </h4>
      ...
      {totalPagesAffichees > 1 && (
        <div className="arrowDiv" style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: "16px", padding: "16px" }}>
          <button onClick={() => setPageAffichee((p) => Math.max(1, p - 1))} disabled={pageAffichee === 1}>
            <i className="fa-solid fa-arrow-left"></i>
          </button>
          <span>
            Page {pageAffichee} / {totalPagesAffichees}
          </span>
          <button onClick={() => setPageAffichee((p) => Math.min(totalPagesAffichees, p + 1))} disabled={pageAffichee === totalPagesAffichees}>
