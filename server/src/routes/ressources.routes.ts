...
router.get("/:id", async (req: Request, res: Response) => {
  ...
  if (!ressource)
    return res
      .status(404)
      .json({ erreur: "Erreur: La ressource est introuvable." });
  res.json(ressource);
});
...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!titre || !contenu || !url || !categorie || !type || !duree || !niveau)
      return res
        .status(400)
...
router.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const ressourceExiste = await prisma.ressource.findUnique({
      ...
      if (!ressourceExiste)
        return res
          ...
          .json({
            erreur:
              "Erreur: Ressource inexistante, impossible d'ajouter aux favoris.",
          });
      ...
      if (favori)
        return res
          .status(409)
          .json({ erreur: "Cette ressource est déjà dans vos favoris!" });

      const nouveauFavori = await prisma.favori.create({
        data: { utilisateurId, ressourceId },
      });
...
router.delete(
  ...
  async (req: Request, res: Response) => {
    ...
    if (!favori)
      return res
        .status(404)
        .json({ erreur: "Erreur: Favori introuvable, impossible d'effacer." });
    if (favori.utilisateurId != utilisateurId)
      return res
        .status(403)
    ...
    try {
      ...
      if (!dernierJournal)
        return res
          ...
          .json({
            message:
              "Erreur: Aucune entrée de journal trouvé. Génération de suggestions impossibles.",
          });
